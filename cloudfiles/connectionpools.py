...
class S3ConnectionPool(ConnectionPool):
  ...
  def _create_connection(self, secrets=None, endpoint=None):
    ...
    return boto3.client(
      ...
      # should probably prioritize AWS_REGION over AWS_DEFAULT_REGION, but
      # that would change the behavior more dramatically.
      region_name=secrets.get('AWS_DEFAULT_REGION', secrets.get('AWS_REGION', 'us-east-1')),
      **additional_args
    )
